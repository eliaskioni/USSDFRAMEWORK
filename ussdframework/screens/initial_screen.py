from ussdframework.core import UssdHandlerAbstract, load_yaml
...
class InitialScreen(UssdHandlerAbstract):
    ...
    It is the screen all new ussdframework session go to.
    ...
    Sometimes you want to send ussdframework session to some 3rd party application when
    ...
    Below is an example of how to schedule a ussdframework report session after 15min
    ...
    example:
        ...
        Lets explain the variables in ussd_report_session
            - session_key ( Mandatory )
                response of ussdframework report session would be saved under that key
                in session store
                
            - request_conf ( Mandatory )
                Those are the parameters to be used to make request to 
                report ussdframework session
            
            - validate_response ( Mandatory )
                After making ussdframework report request the framework will evaluate
                your options and if one of them is valid it would 
                mark session as posted (This is used to avoid double ussdframework
                submission)
            ...
            - async_parameters ( Optional )
                This is are the parameters used to make ussdframework request
    ...
    def handle(self):
        ...
        if isinstance(self.screen_content, dict):
            ...
            # create ussdframework variables defined int the yaml
